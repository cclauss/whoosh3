..
   Keep this page in sync with MIGRATING.md at the repository root. The root
   file is the GitHub-facing copy; this page is the search-indexed docs copy.
...
`Whoosh <https://github.com/whoosh-community/whoosh>`_ by Matt Chaput and the
later `whoosh-reloaded <https://github.com/Sygil-Dev/whoosh-reloaded>`_ fork.
...
#. **Uninstall the old distribution** to avoid two packages fighting over the
...
#. **Run your existing code.** No import or call-site changes are expected.
...
Zero runtime dependencies again
-------------------------------
...
* ``loguru`` -- removed; it was pulled in but not actually used for anything
  you relied on.
...
installed ``cached-property`` or ``loguru`` *only* for whoosh-reloaded, you
can drop them.
...
* **MultiFilter on an empty token stream** no longer raises ``StopIteration``
  (which under modern Python surfaces as a ``RuntimeError``). Empty input now
  correctly yields no tokens. This bites null/empty queries used with custom
  tokenizers.
* **RamStorage.temp_storage() no longer touches disk.** It previously returned
...
* **Public API:** ``create_in``, ``open_dir``, ``Schema``, field types,
  ``writer()``, ``searcher()``, ``QueryParser`` / ``MultifieldParser``,
  scoring (``BM25F`` default), highlighting, spelling/``suggest``, faceting --
  all unchanged.
...
**Do I have to rename my imports to** ``whoosh3`` **?**
   No. Always ``import whoosh``. Only the PyPI/``pip install`` name is
   ``whoosh3``.

**Can I install** ``whoosh3`` **alongside the old** ``Whoosh`` **?**
   Not safely -- both provide the ``whoosh`` import package, so pip will let
   one shadow the other. Uninstall the old one first (step 1 above).
...
**I maintain a library that depends on** ``Whoosh`` **. What should I pin?**
   Depend on ``whoosh3`` and keep importing ``whoosh``. If you must support
   both the old and new distributions during a transition, that is possible
   but not recommended long-term; prefer moving to ``whoosh3``.
...
`paperless-ngx <https://github.com/paperless-ngx/paperless-ngx>`_ -- relies
on: an ``AsyncWriter``, a mixed
``TEXT`` / ``KEYWORD`` / ``DATETIME`` / ``NUMERIC`` / ``BOOLEAN`` schema,
``MultifieldParser`` with the ``DateParserPlugin`` for date-range queries,
``TF_IDF`` scoring, and ``HtmlFormatter`` highlighting. If any of those break,
our build goes red before a release ships. Migrating a project of that shape
is expected to be a one-line dependency change.

If something *did* break when you migrated, that's a bug we want to know
about -- please
`open an issue <https://github.com/priya-sundaram-dev/whoosh/issues>`_ with a
minimal reproduction. Migration is supposed to be painless, and regressions
